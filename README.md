...
No painel do projeto: **Connect → "Session pooler"** (ou **Project Settings → Database**).
Ela tem este formato:
```
postgresql://postgres.SEU_REF:SUA_SENHA@aws-0-sa-east-1.pooler.supabase.com:5432/postgres
```
 
> ⚠️ Use sempre a versão **Session pooler**. A conexão direta (`db.SEU_REF.supabase.co`) usa
> IPv6 e pode falhar em algumas redes e no deploy.
...
> Alternativa: defina a variável de ambiente `DATABASE_URL` antes de rodar.
...
> No Windows (PowerShell), garanta o ambiente de desenvolvimento antes de rodar:
